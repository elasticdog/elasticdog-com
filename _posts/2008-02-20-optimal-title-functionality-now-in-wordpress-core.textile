...
---
I wrote "Optimal Title":http://elasticdog.com/2004/09/optimal-title/ three and a half years ago to help fix a blatant flaw in "WordPress's":http://wordpress.org default usability and search engine optimization (SEO). Since then, the plugin has been downloaded by tens of thousands of people and written about by hundreds of bloggers. Soon, the plugin will become obsolete...

h3. A Change in WordPress v2.5

As of the beginning of this year, a "changeset":http://trac.wordpress.org/changeset/6538 was checked in to WordPress's SVN(Subversion) repository which adds a "separator location" argument to the built-in @wp_title()@ function. What that means, is that when WordPress v2.5 is released around March 10th of this year, users will be able to change the ordering of their HTML(Hypertext Markup Language) title tags without having to use a third-party plugin.

The "developers decided":http://trac.wordpress.org/ticket/4407 not to change the default behavior of @wp_title()@, and instead added the additional argument to the function in order to prevent breaking existing themes that rely on the current behavior.

h3(#change). How to Make the Change
...
Using the simplified example from the "original Optimal Title article":http://elasticdog.com/2004/09/optimal-title/#usage, you should currently have something like this in your theme's <kbd>header.php</kbd> file:

bc. <title><?php optimal_title(); ?> <?php bloginfo('name'); ?></title>
...
bc. <title><?php wp_title('&raquo;',TRUE,'right'); ?> <?php bloginfo('name'); ?></title>

That's it!

h3(#parameters). Parameters

The "wp_title codex page":http://codex.wordpress.org/Template_Tags/wp_title won't be updated until the next release, however, here is how @wp_title()@ will work after WordPress v2.5 is out:

bc. <?php wp_title('separator', display, 'seplocation'); ?>
