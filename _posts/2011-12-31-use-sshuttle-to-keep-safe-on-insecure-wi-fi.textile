...
Once sshuttle is built, we just make symlinks so the binary, man page, and GUI app are in the proper places:

{% highlight bash %}
$ ln -s /usr/local/libexec/sshuttle/sshuttle /usr/local/bin
$ mkdir -p /usr/local/share/man/man8
$ ln -s /usr/local/libexec/sshuttle/sshuttle.8 /usr/local/share/man/man8
$ ln -s /usr/local/libexec/sshuttle/ui-macos/Sshuttle\ VPN.app /Applications
{% endhighlight %}
...
We can update the flag by running:
...
_*NOTE:* Sshuttle will honor your @~/.ssh/config@ connection settings, but you can also manually specify a different username/port, if necessary:_
...
The __<notextile>Sshuttle VPN.app</notextile>__ GUI is pretty self-explanatory, but again, it's not without its complications...

h2(#bugs). Known Bugs

Version 0.54 has "a bug":http://groups.google.com/group/sshuttle/browse_thread/thread/6603f9d17ae8eec4 where sometimes sshuttle won't properly revert the firewall rules when tearing down the proxy. This means *all* network connectivity will be broken afterward until you flush out the rules manually. This happens every time you disconnect when using the GUI, but I've also seen a report of it happening with the CLI(Command-Line Interface) as well. Personally, I've never had any issues with the CLI, but your mileage may vary. Hopefully, the underlying issue will be fixed soon so you no longer have to worry about it.
