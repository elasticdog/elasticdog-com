...
ElasticDog.com is hosted on a VPS(Virtual Private Server) from "Linode":http://www.linode.com/ along with a number of my other sites. The server runs "Arch Linux":http://www.archlinux.org/ and has its configuration managed via "Chef":http://wiki.opscode.com/display/chef/Home using the "Opscode Platform":http://www.opscode.com/. The site itself is completely static, but uses a combination of "Jekyll":http://github.com/mojombo/jekyll with "RedCloth":http://redcloth.org/ to generate the HTML(Hypertext Markup Language), and "Compass":http://compass-style.org/ with "Sass":http://sass-lang.com/ to generate the CSS(Cascading Style Sheets).
